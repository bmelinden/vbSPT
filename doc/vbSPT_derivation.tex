...
\ST  \verb+W.E(m).wPi(j)+ & $\mean{\delta_{j,s_1}}$ & \eqref{VBM_pi} \\
  \hline
\ST  \verb+W.E(m).wA(j,k)+& $\sum_{t=1}^{T-2}\mean{\delta_{j,s_t}\delta_{k,s_{t+1}}}$ & \eqref{VBM_B},\eqref{VBM_a}\\
  \hline
...
\hline
\verb+W.SA+ & State $j$ is in aggregate \verb+W.SA(j)+.\\
\hline
\verb+W.F+ & Total lower bound $F$.\\
\hline
\verb+W.Fterms+ & Various contributions to $F$ (for debugging). \\
\hline
\verb+W.T(m)+ & Length of trajectory $m$. Note that this counts \\
              &the number of positions, and hence trajectory j will\\
              & contain \verb+W.T(m)-1+ steps, and \verb+W.T(m)-2+
              transitions.\\
\hline
\end{tabular}\end{center}
\item Interesting estimates are supplied to describe the output and
  ...
  refers to trajectory number. \ML{Entries somewhat outdated.}
