...
averages.
\subsection{Priors}
The current version of the code lets the user specify a mean value and
a prior strength for the diffusion constants, from which $\tilde c_j$
and $\tilde n_j$ are computed, same for all model sizes.

The prior for the initial state distribution is chosen as a flat
Dirichlets, same for all model sizes, and the same is true for the
jump probabilties, i.e., 
$\tilde w_j^{(\vec{\pi})}=1$ and $\tilde w_{jk}^{(\matris{B})}=1$.

Dwell time priors are specified by mean value and standard deviation
of the prior dwell times, from which pseudocounts are computed using
Eq.~\eqref{eq:wjka}.

For more precise control of the priors, modify the
function \texttt{VB3\_createPrior.m}.
