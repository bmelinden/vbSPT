...
To start with, we will feed the model
Eqs. (\ref{eq:efactor1}-\ref{xmodel1}), and with the new
parameterization,
\begin{multline}
\label{eq:efactor1b}
...
and the beta distribution is the special case of two components $a_j$
and $1-a_j$.  Inspection of Eq.~\eqref{Maverage} reveals that
...
\label{VBM_B}
  q(\matris{B})=&\prod_j
  \Dir(B_{j,:}|w_{j,:}^{(\matris{A})}),&
  w_{jk}^{(\matris{A})}=&\tilde w_{jk}^{(\matris{A})}
  +\sum_{t=1}^{T-2}\mean{\delta_{j,s_t}\delta_{k,s_{t+1}}}_{q(s)},\\
\label{VBM_a}
  q(\vec{a})=&\prod_j
  \beta(a_j|w_{jj}^{(\matris{A})},w_{j0}^{(\matris{A})}-w_{jj}^{(\matris{A})}),&
  w_{j0}^{(\matris{A})}=&\sum_{k}w_{jk}^{(\matris{A})}
\end{align}
where $\tilde w_j^{(\vec{\pi})}$ and $\tilde w_{jk}^{(\matris{A})}$
are pseudocounts in the prior distributions.  The total number of
counts (for each distribution) is called the prior strength. Note that
$B_{jj}=0$, hence $w_{jj}^{(\matris{A})}$ does not contribute to
$q(\matris{B})$.  The following averages\cite{Beal2003} will turn out
to be useful:
\begin{align}
  \mean{\ln \pi_i}_{q(\vec{\pi})}=&\psi(w_i^{(\vec{\pi})})-\psi(w_0^{(\vec{\pi})}),
  &w_0^{(\vec{\pi})}=&\sum_{i=1}^Nw_i^{(\vec{\pi})},\label{lnpiaverage}\\
  \mean{\ln a_k}_{q(\vec{a})}=&\psi(w_{kk}^{(\matris{A})})-\psi(w_{k0}^{(\matris{A})}),\\
  \mean{\ln (1-a_k)}_{q(\vec{a})}=&\psi(w_{k0}^{(\matris{A})}-w_{kk}^{(\matris{A})})-\psi(w_{k0}^{(\matris{A})}),\\
  \mean{\ln B_{kj}}_{q(\matris{A})}=&\psi(w_{kj}^{(\matris{A})})
    -\psi(w_{k0}^{(\matris{A})}-w_{kk}^{(\matris{A})}),
\end{align}
\begin{align}
  \pi^*_{i\;q(\vec\pi)}=&\frac{w_i^{(\vec\pi)}-1}{w_0^{(\vec\pi)}-N}, &
  \mean{\pi_i}_{q(\vec{\pi})}=&\frac{w_i^{(\vec\pi)}}{w_0^{(\vec\pi)}}, \\
  a^*_{i\;q(\vec\pi)}=&\frac{w_{ii}^{(\matris{A})}-1}{w_{i0}^{(\matris{A})}-2}, &
  \mean{a_i}_{q(\vec{\pi})}=&\frac{w_{ii}^{(\matris{A})}}{w_{i0}^{(\matris{A})}},\\
  %\\&w_{k0}^{(\matris{A})}=&\sum_{j=1}^Nw_{kj}^{(\matris{A})},\\
%  \mean{\ln A_{kj}}_{q(\matris{A})}=&\psi(w_{kj}^{(\matris{A})})
%    -\psi(w_{k0}^{(\matris{A})}),
%  &w_{k0}^{(\matris{A})}=&\sum_{j=1}^Nw_{kj}^{(\matris{A})},\\
  B^*_{jk\;q(\matris{A})}=&\frac{w_{jk}^{(\matris{A})}-1}{w_{j0}^{(\matris{A})}-w_{jj}^{(\matris{A})}-N+1}&
  \mean{B_{jk}}_{q(\matris{B})}=&\frac{w_{jk}^{(\matris{A})}}{w_{j0}^{(\matris{A})}-w_{jj}^{(\matris{A})}}\\
  \Var[\pi_i]_{q(\vec{\pi})}=&
...
in the distribution $q(\cdot)$. {\bf ML got this far!}
