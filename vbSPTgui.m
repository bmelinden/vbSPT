...
fprintf(fID, '%s\r\n',...
    ...
    '',...
    '% initial state',...
    'prior_type_Pi = ''flat''; % flat distribution for initial states',...
    '',...
    '% transition matrix',...    
    '% units: same time units as timestep.',...
    'prior_type_A = ''dwell_Bflat''; % flat distribution for jump probabilities',...
    'prior_tD = 10*timestep;       % prior dwell time in [s].',...
    'prior_tDstd = 100*prior_tD;   % standard deviation of prior dwell times [s]. A large std corresponds to an unformative prior.',...
    '');
